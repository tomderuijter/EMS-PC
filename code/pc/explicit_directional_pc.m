function [directed_graph] = explicit_directional_pc(undirected_graph, sepset)
...
% Majority vote whether adding Y to sepset(X,Z) makes X,Z dependent.
function [CD] = explicit_dependence(X,Y,Z,C,T,sepsets,alpha)

    sets = sepsets{X,Z};
    N_sets = length(sets);
    dep_count = 0;
    for ii = 1:N_sets
        if(~cond_indep_fisher_z(X,Z,[sets{ii},Y],C,T,alpha))
