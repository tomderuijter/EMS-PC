function [directed_graph] = explicit_directional_pc(undirected_graph, sepset, explicit, varargin)
...
% 2 - arrow
% 3 - dot
% For example, (x,y)=2 and (y,x)=0 means x -> y
...
% turn undirected edges into equivalence classes
directed_graph = directed_graph .* 3;

fprintf('Finding V-structures...');
dtime=cputime;

[directed_graph, nrEdges] = find_v_structures(undirected_graph, directed_graph, sepset, explicit, varargin{:});
...
% For graphical purposes, convert classes to neighbours.
directed_graph(directed_graph == 3) = 1;

end


function [directed_graph, nrEdges] = find_v_structures(undirected_graph, directed_graph, sepset, explicit, varargin)
N = size(undirected_graph, 1);
nrEdges=0;
...
% When multiple sepsets are provided, performs a majority vote.
% Returns whether X and Z are dependent.
% varargin: for fisher_z_independence test ('fisher_z_independence',C,T,alpha)
function [CD] = explicit_dependence(sepset,x,y,S,varargin)

    cond_indep = varargin{1};
    sets = sepset{x,y};
    N_sets = length(sets);
    dep_count = 0;
    for ii = 1:N_sets
        if(~feval(cond_indep, x, y, [sets{ii},S], varargin{2:end}))
