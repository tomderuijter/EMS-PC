...
\While{Edges can still be oriented}
  ...
  \EndIf
  \If{there are edges $X \srightarrow Y$ and $Y \sleftline Z$, the latter is not an edge $Y \sleftarrow Z$, and $X$ and $Z$ are not connected}
    \State orient $Y \sline Z$ as $Y \rightarrow Z$
  \EndIf
\EndWhile
\State Apply other directionality rules from Spirtes et al. \cite[p.165-167]{spirtes2000} %TODO uitschrijven?
