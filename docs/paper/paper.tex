...
The second part orients edges in this graph $G$ based on the separating sets. In this pseudocode, an edge with a star at the end means that that end may be either an arrowhead, an "o" or an empty mark. When an edge is oriented as an edge with a star at the end, this denotes that this end is not changed.
\begin{algorithm}
...
\Function{Directional\_PC}{$G$, $sepsets$}
  ...
  \ForAll{triples $X$, $Y$, $Z$ with the structure $X\sline Y \sline Z$ in $G$ (where $X$ and $Z$ are not connected)}
...
Two changes have been made in this adaption with respect to modified PC as described by Spirtes et al \cite{Spirtes2000}. In the algorithm as described in section ~\ref{sec:pseudocode}, the search for a separating sets for $X$ and $Y$ stops when such a set is found; instead, EMS continues the search for more separating sets of the same order.
