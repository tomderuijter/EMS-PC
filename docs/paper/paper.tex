...
%\begin{algorithm}[PC-algorithm]
\begin{algorithmic}
\State Starting state: connect all pairs of point in undirected graph $G$.\\
\State n := 0
\While{$n < |\operatorname{Adjecencies}(X)\setminus\{Y\}| $}
  \ForAll{connected pair $X$, $Y$}
    \ForAll{subset $S$ of $\operatorname{Adjacencies(X)}$ for which $|S| = n$}
      \State test whether $X$ and $Y$ are d-separated given $S$ (Fisher-Z-test)
