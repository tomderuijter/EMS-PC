...
\end{itemize}
If $X$ and $Y$ are d-separated given $S$, $S$ is called a separating set of $X$ and $Y$. Every two independent variables have a separating set; if they are unconditionally independent, his is the empty set.
...
A set of rules for finding a causal relation between nodes, using the undirected graph, are used in the PC-algorithm.

One of these rules attempts to find so-called v-structures, which are of the form $X \rightarrow Z \leftarrow Y$.
...
The other rules are based on arrows that have already been found.
\begin{itemize}
...
\section{Methods}
The original PC algorithm assumes causal sufficiency, excluding common confounders between nodes.
An adaptation of this algorithm by Spirtes et al. yielded Modified PC, which returns a Markov equivalence class, rather than one specific instance of it.
In literature it is this algorithm that is usually cited.

As mentioned earlier, PC Algorithm has stability problems as the final solution depends on an initial ordering of vertices.
This problem decreases when considering Modified PC; as latent common confounders are not excluded by design bi-directional arrows occur in subgraphs where original PC would attempt to direct a single arrow in either direction depending on node ordering.
...
However, this method also require a significant amount of additional conditional independence tests.

Based on similar ideas, we answer whether an unshielded triple is faithful based on already done conditional independence tests. 
